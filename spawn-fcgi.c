...
static int bind_socket(const char *addr, unsigned short port,
					   const char *unixsocket, uid_t uid, gid_t gid,
...
static int fcgi_spawn_connection(char *appPath, char **appArgv, int fcgi_fd,
								 int fork_count, int child_count, int pid_fd,
								 int nofork)
{
	int status, rc = 0;
	...
	while (fork_count-- > 0) {
		...
		case 0:{
				...
				if (child_count >= 0) {
					snprintf(cgi_childs, sizeof(cgi_childs),
				...
				if (appArgv) {
					...
					char *b =
						malloc((sizeof("exec ") - 1) + strlen(appPath) + 1);
					strcpy(b, "exec ");
					strcat(b, appPath);
...
static int find_user_group(const char *user, const char *group, uid_t * uid,
						   gid_t * gid, const char **username)
{
	uid_t my_uid = 0;
	gid_t my_gid = 0;
...
int main(int argc, char **argv)
{
	...
	while (-1 !=
		   (o = getopt(argc, argv, "c:d:f:g:?hna:p:b:u:vC:F:s:P:U:G:M:S"))) {
		switch (o) {
		case 'f':
			fcgi_app = optarg;
			break;
	...
	if (i_am_root) {
		...
		if (-1 ==
			find_user_group(username, groupname, &uid, &gid, &real_username))
			return -1;

		if (-1 ==
			find_user_group(sockusername, sockgroupname, &sockuid, &sockgid,
							NULL))
			return -1;
		...
		if (sockbeforechroot
			&& -1 == (fcgi_fd =
					  bind_socket(addr, port, unixsocket, sockuid, sockgid,
								  sockmode, backlog)))
			return -1;
		...
		if (-1 ==
			(fcgi_fd =
			 bind_socket(addr, port, unixsocket, 0, 0, sockmode, backlog)))
			return -1;
	}
	...
	return fcgi_spawn_connection(fcgi_app, fcgi_app_argv, fcgi_fd, fork_count,
								 child_count, pid_fd, nofork);
}
